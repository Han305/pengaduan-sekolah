...
@section('body')
    ...
    @endif
    <div class="pt-3">
        <a href="{{ route('laporan.create') }}" class="btn btn-primary btn-md">
            + Tambah Laporan
        </a>
    </div>
            ...
            <tbody>
                ...
                @foreach ($posts as $item)
                    <tr>
                        ...
                        <td>{{ $item->status }}</td>
                        <td>
                            <a href="{{ route('laporan.edit', ['post' => $item->id]) }}"
                                class="btn btn-warning btn-sm">Edit</a>
                            <a href="{{ route('laporan.destroy', ['post' => $item->id]) }}"
                                class="btn btn-danger btn-sm" onclick="return deleteConfirm()">Hapus</a>
                            <a href="{{ route('laporan.tanggapi', ['id' => $item->id]) }}" class="btn btn-success btn-sm">Tanggapi</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
